...
| `check-isglobal.sh`         | `is:global` anywhere in `src/pages/**/*.astro`.                                                                                                                                                                                                                                                                                                                                                                                             | **Exit 1.** Two real hits: `src/pages/ui-kits/portfolio.astro` and `src/pages/ui-kits/blog.astro` (the `<style is:global>` composition blocks). Both need global scope because their `body.p-dark`/`body.b-dark` selectors would otherwise be defeated by Astro's scoped-style hashing. Still a rule exception, not a free pass — take it to `styleguide-change-control`. |
| `check-single-gradient.sh`  | Every `linear-gradient()`/`radial-gradient()` under `src/`, minus the two sanctioned brand motifs: the blog-nav sweep (`src/styles/components.css` `.lp-blog-nav`, `135deg, var(--lp-ink-2) 0%, var(--lp-blog-blue-700) 100%`) and the portfolio hero blueprint grid (`src/pages/ui-kits/portfolio.astro` `.p-hero`, two `--lp-grid-line` lines). The hero's rotating amber radar is a `conic-gradient()` and is not matched by this check. | **Exit 0** on the post-rebrand tree — only the two whitelisted motifs exist. A hit here now means a genuine THIRD gradient; gate it through `styleguide-change-control`.                                                                                                                                                                                                  |
...
- The two sanctioned gradient declarations haven't moved: `grep -n 'linear-gradient' src/styles/components.css src/pages/ui-kits/portfolio.astro`.

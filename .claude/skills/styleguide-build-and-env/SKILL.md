...
before it ships — this skill tells you _how_ to change a config axis, not whether you're allowed to.
...
4. If the change touches `public/_headers` specifically, the CSP _rationale_ (why each directive
...
vars are read by the site code itself; `NODE_VERSION=22` is a Cloudflare Pages _build-environment_
...
| `nvm use` refuses to switch Node (verified 2026-07-31) | `nvm use 22` → "Your user's .npmrc file (${HOME}/.npmrc) has a `globalconfig` and/or a `prefix` setting, which are incompatible with nvm"; the suggested `--delete-prefix` command fails identically, and so does `nvm ls` | On this machine nvm has **zero** Node versions installed (`ls ~/.nvm/versions/node` is empty) and `~/.npmrc` sets `prefix=${HOME}/.local`, so §1's `nvm install && nvm use` line does not work as written — Node comes from Homebrew (24.x / npm 11.x). If the goal is only to match CI's **npm** (e.g. regenerating `package-lock.json` the way the Node-22/npm-10 workflow and CF Pages would), don't switch Node at all: run `npx -y npm@10.9.2 install` / `npx -y npm@10.9.2 ci --dry-run`. Full runbook: `nvm-use-blocked-by-npmrc-prefix-pin-npm-via-npx` |
